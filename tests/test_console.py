#!/usr/bin/python3
""" Defines a unittest module for the console.py"""
import json
import MySQLdb
import os
import sqlalchemy
...
class TestHBNBCommand(unittest.TestCase):
    """Unittest for the HBNB Command class"""
    @unittest.skipIf(
        os.getenv('HBNB_TYPE_STORAGE') == 'db', 'FileStorage test')
    def test_fs_create(self):
        """Testing the create command with the file storage"""
    ...
    def test_db_create(self):
        """Testing the create command with the database storage"""
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
